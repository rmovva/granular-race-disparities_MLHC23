...
# Names of Charlson comorbidity variables
cci_var_map = {
...
# Names of Elixhauser comorbidity variables
eci_var_map = {
...
def comorbidity_set(icd, version, mapping='charlson'):
    c_list = []
    if mapping == 'charlson':
...
def comorbidity_dict(icd, version, mapping='charlson'):
    map_set = comorbidity_set(icd, version, mapping)
...
def extract_icd_list(df_edstays, df_diagnoses, df_admissions, output_path, timerange = 365*5, version = 'v9_3digit'):
...
def comorbidity(df_master, df_diagnoses, df_admissions, timerange):
    timerange = timedelta(days=timerange)
    ...
    for i, row in df_master.iterrows():
        stay_ids.append(row['stay_id'])
        if i % 10000 == 0:
            print('Processing: %d/%d' % (i, len(df_master)), end='\r')
        # If new subject, find the start and end index of same subject in sorted admission df
        if row['subject_id'] != prev_subject:
            j_start = j_end
            while j_start < len(df_diagnoses_sorted) and df_diagnoses_sorted['subject_id'][j_start] < row['subject_id']:
    ...
    for i, code in enumerate(diagnoses):
        cci_eci.append({'stay_id':stay_ids[i], **comorbidity_dict(code, versions[i], mapping='charlson'), **comorbidity_dict(code, versions[i], mapping='elixhauser')})
